...
function simulateWebSocketConnected(client: Client) {
  client.onConnect(new FrameImpl({
    command: 'MESSAGE',
    headers: {}
  }));
}
...
function simulateStompErrorAtDestination(client: Client, destination: string) {
  client.onStompError(new FrameImpl({
    command: 'ERROR',
    body: destination
  }));
}
...
describe('Correlate Errors', () => {
    ...
    beforeEach(() => {
      ...
      // subscribed
      spyOn(client, 'subscribe').and.callFake((destination: string, callback: messageCallbackType) => {
        callback(new MessageImpl({
          command: 'MESSAGE',
          body: destination,
        }));
